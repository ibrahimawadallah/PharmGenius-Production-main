...
async function processICD10Fast() {
  ...
  // Load UAE drug list
  const csvPath = path.join(__dirname, '..', 'public', 'UAE drug list.csv');
  const csvContent = fs.readFileSync(csvPath, 'utf8');
  const parsed = Papa.parse(csvContent, { header: true, skipEmptyLines: true });
  
  console.log(`📊 Loaded ${parsed.data.length} drugs from UAE list`);
  
  // Prioritize common drugs (first 1000 entries)
  const prioritizedDrugs = parsed.data.slice(0, 1000);
  ...
  for (const row of prioritizedDrugs) {
    const drugName = row['Drug Name'] || row['drug_name'] || row['name'];
    if (!drugName) continue;
    
    processed++;
    console.log(`[${processed}/${prioritizedDrugs.length}] Processing: ${drugName}`);
